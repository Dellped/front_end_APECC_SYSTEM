...
export default function ListOfExit() {
  ...
  const filteredRecords = exitRecords.filter((rec) =>
    ...
    rec.branchName.toLowerCase().includes(search.toLowerCase())
  );

  const headers = [
    'No.', 'ID No.', 'DATE OF EXIT', 'NAME', 'DESIGNATION', 'Savings', 
    ...
    'Gadget Loan', 'Malasakit Loan', 'Grand Total', 'BRANCH CODE', 'BRANCH NAME', 
    'Reason', 'variance', 'CLEARANCE', 'SYSTEM', 'Rebates', 'Remarks'
  ];

  const handleExportCSV = () => {
    const rows = filteredRecords.map(r => [
      r.no, r.idNo, r.dateExit, r.name, r.designation, r.savings, 
      ...
      r.gadgetLoan, r.malasakitLoan, r.grandTotal, r.branchCode, r.branchName, 
      r.reason, r.variance, r.clearance, r.system, r.rebates2, r.remarks
    ]);
            ...
            <TableBody>
              {filteredRecords.map((row, index) => (
                <TableRow key={index} hover sx={{ '&:nth-of-type(even)': { bgcolor: 'rgba(0,0,0,0.01)' } }}>
                  ...
                  <TableCell sx={{ fontSize: '0.75rem', fontWeight: 600 }}>{row.idNo}</TableCell>
                  ...
                  <TableCell sx={{ fontSize: '0.75rem', whiteSpace: 'nowrap' }}>{row.branchName}</TableCell>
                  ...
                  <TableCell sx={{ fontSize: '0.75rem', fontStyle: 'italic' }}>{row.remarks}</TableCell>
                </TableRow>
              ))}
              {filteredRecords.length === 0 && (
                <TableRow>
                  <TableCell colSpan={30} align="center" sx={{ py: 6 }}>
