...
import { hrAuditLogs } from '../../data/mockData';
...
export default function HRAuditTrail() {
  ...
  const filteredLogs = hrAuditLogs.filter(log => {
  ...
  const uniqueModules = ['All', ...new Set(hrAuditLogs.map(log => log.module))];
    ...
    <Box className="page-container">
      <Box sx={{ mb: 4 }}>
        ...
        <Typography variant="body2" sx={{ color: 'text.secondary', fontWeight: 500 }}>
          Track all actions and modifications made within the HR module.
        </Typography>
      </Box>
          ...
          <Box sx={{ display: 'flex', gap: 2, mb: 4, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <Stack direction="row" spacing={1}>
              <Button size="small" variant="outlined" startIcon={<CsvIcon />}
                onClick={() => exportToCSV(['Date & Time','User','Module','Action','Details'], filteredLogs.map(l => [l.date, l.user, l.module, l.action, l.details]), 'hr_audit_trail')}
                sx={{ borderRadius: 2, fontSize: '0.75rem' }}>CSV</Button>
              <Button size="small" variant="outlined" startIcon={<PdfIcon />}
                onClick={() => exportToPDF('HR Audit Trail', ['Date & Time','User','Module','Action','Details'], filteredLogs.map(l => [l.date, l.user, l.module, l.action, l.details]))}
                sx={{ borderRadius: 2, fontSize: '0.75rem' }}>PDF</Button>
              <Button size="small" variant="outlined" startIcon={<PrintIcon />}
                onClick={() => printTable('HR Audit Trail', ['Date & Time','User','Module','Action','Details'], filteredLogs.map(l => [l.date, l.user, l.module, l.action, l.details]))}
