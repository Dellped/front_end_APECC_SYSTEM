...
export default function LeaveApplicationForm({ employee, onCancel, onSubmit }) {
  ...
  return (
    <Box sx={{ p: 2, bgcolor: '#f5f5f5', minHeight: '100vh', display: 'flex', justifyContent: 'center' }}>
      <Paper sx={{ p: 4, width: '100%', maxWidth: 850, bgcolor: '#fff', border: '1px solid #ddd' }}>
        {/* Header */}
        ...
        <Box sx={{ border: '2px solid #000', p: 0 }}>
            ...
            <Box sx={{ p: 1, borderBottom: '1px solid #000' }}>
                <Box sx={{ display: 'flex', alignItems: 'flex-end', gap: 1, mb: 1 }}>
                    ...
                    <TextField variant="standard" fullWidth value={formData.addressOnLeave} onChange={(e) => setFormData({...formData, addressOnLeave: e.target.value})} />
                </Box>
                <Box sx={{ display: 'flex', alignItems: 'flex-end', gap: 1 }}>
                    <Typography variant="body2" sx={{ fontWeight: 600, minWidth: 180 }}>Person responsible in my absence</Typography>
                    <TextField variant="standard" fullWidth value={formData.personResponsible} onChange={(e) => setFormData({...formData, personResponsible: e.target.value})} />
                </Box>
            </Box>
            ...
            <Grid container sx={{ borderBottom: '1px solid #000' }}>
                <Grid item xs={6} sx={{ p: 1, borderRight: '1px solid #000' }}>
                     <Box sx={{ display: 'flex', alignItems: 'flex-end', gap: 1 }}>
                        ...
                        <TextField type="date" variant="standard" value={formData.startDate} onChange={(e) => setFormData({...formData, startDate: e.target.value})} />
                        <Typography variant="body2">to</Typography>
                        <TextField type="date" variant="standard" value={formData.endDate} onChange={(e) => setFormData({...formData, endDate: e.target.value})} />
                    </Box>
                </Grid>
                <Grid item xs={6} sx={{ p: 1 }}>
                    <Box sx={{ display: 'flex', alignItems: 'flex-end', gap: 1 }}>
                        <Typography variant="body2" sx={{ fontWeight: 600 }}>Total No. Of Days</Typography>
                        <TextField variant="standard" fullWidth value={formData.totalDays} onChange={(e) => setFormData({...formData, totalDays: e.target.value})} />
                    </Box>
                </Grid>
            </Grid>
            ...
            <Box sx={{ p: 1, borderBottom: '1px solid #000', display: 'flex', alignItems: 'center', gap: 2 }}>
                ...
                {['Sick', 'Earn', 'Annual', 'Maternity/Patern', 'Others'].map((type) => (
                    <FormControlLabel
                        key={type}
                        control={<Checkbox size="small" checked={formData.designation === type} onChange={() => setFormData({...formData, designation: type})} />}
            ...
            <Box sx={{ p: 1, borderBottom: '1px solid #000' }}>
                ...
                <TextField multiline rows={2} fullWidth variant="outlined" sx={{ '& .MuiOutlinedInput-root': { borderRadius: 0 } }} value={formData.purpose} onChange={(e) => setFormData({...formData, purpose: e.target.value})} />
            </Box>
            ...
            <Grid container sx={{ borderBottom: '2px solid #000' }}>
                <Grid item xs={6} sx={{ p: 1, borderRight: '1px solid #000' }}>
                    <Box sx={{ display: 'flex', alignItems: 'flex-end', gap: 1, mt: 2 }}>
                        ...
                        <TextField variant="standard" fullWidth sx={{ borderBottom: '1px solid #000' }} />
                    </Box>
                </Grid>
                <Grid item xs={6} sx={{ p: 1 }}>
                    <Box sx={{ display: 'flex', alignItems: 'flex-end', gap: 1, mt: 2 }}>
                        <Typography variant="body2" sx={{ fontWeight: 600 }}>Date</Typography>
                        <Typography variant="body2" sx={{ flex: 1, borderBottom: '1px solid #000', px: 1 }}>{formData.date}</Typography>
                    </Box>
                </Grid>
            </Grid>
